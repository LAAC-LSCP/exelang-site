+++
date = '2026-04-03T00:48:57+02:00'
draft = false
title = 'Presentation Recordings'
headless = true
+++

## Presentation Recordings

Seminars, tutorials, and online workshops.

{{< link-box icon="camera" >}}
{{< link-box-header >}}

### Introduction to ExELang Tools

Overview of the ExELang ecosystem and how to get started
{{< /link-box-header >}}
{{< link-box-icon href="https://sdrive.cnrs.fr/s/QGtMqG4gDmZgAPz" icon="right-arrow" >}}
Watch Recording
{{< /link-box-icon >}}
{{< /link-box >}}

{{< link-box icon="camera" >}}
{{< link-box-header >}}

### Advanced VTC2 Workshop

Deep dive into voice classification techniques
{{< /link-box-header >}}
{{< link-box-icon href="https://www.google.com/" icon="right-arrow" >}}
Watch Recording
{{< /link-box-icon >}}
{{< /link-box >}}

{{< link-box icon="camera" >}}
{{< link-box-header >}}

### BabyHubert Tutorial Series

Complete guide to using BabyHubert for infant speech analysis
{{< /link-box-header >}}
{{< link-box-icon href="https://www.google.com/" icon="right-arrow" >}}
Watch Recording
{{< /link-box-icon >}}
{{< /link-box >}}
